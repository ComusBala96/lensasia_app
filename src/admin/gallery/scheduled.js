...
$(document).ready(function () {
    ...
    if ($('#dtGalleryScheduled').length > 0) {
        ...
        let col_draft = [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `<a href="${domain_url}admin/gallery/scheduled/view/${data.uuid}"><span class="p-2 rounded-full shadow-md text-white" style="background-color:#2edcdc;"><i class="fa fa-eye"></i></span></a>
        ...
        makeAjaxDataTable('dtGalleryScheduled', {
            select: false,
            url: 'admin/gallery/scheduled/list',
            columns: col_draft,
            pdf: [0, 1, 2, 3, 4, 5, 6, 7],
        });
    }
...
window.dtGalleryScheduled = (table, api, op) => {
    if ($('.scheduled').length > 0) {
        $('.scheduled').on('click', function () {
            ajaxRequest({
                element: 'scheduled',
                confirm: true,
                script: 'admin/gallery/scheduled/update',
                type: 'request',
                dataType: 'json',
    ...
    downloadPdf({ ...op, btn: 'downloadGalleryScheduledPdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadGalleryScheduledExcel', dataTable: 'yes' });
};