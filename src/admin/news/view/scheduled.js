...
$(document).ready(function () {
    ...
    if ($('#dtScheduled').length > 0) {
        ...
        makeAjaxDataTable('dtScheduled', {
            select: false,
            url: 'admin/news/show/scheduled/list',
            columns: col_draft,
            pdf: [0, 1, 3, 4, 6, 7, 8],
        });
    }
...
window.dtScheduled = (table, api, op) => {
    if ($('.scheduled').length > 0) {
        $('.scheduled').on('click', function () {
            ajaxRequest({
                element: 'scheduled',
                confirm: true,
                script: 'admin/news/show/scheduled/update',
                type: 'request',
                dataType: 'json',
    ...
    downloadPdf({ ...op, btn: 'downloadScheduledPdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadScheduledExcel', dataTable: 'yes' });
};