        ...
        let col_draft = [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `<a href="${domain_url}admin/news/show/feature/view/${data.uuid}"><span class="p-2 rounded-full shadow-md text-white" style="background-color:#2edcdc;"><i class="fa fa-eye"></i></span></a>`;
                },
            },
        ];
        makeAjaxDataTable('dtFeature', {
            select: false,
            url: 'admin/news/show/feature/list',
            columns: col_draft,
            pdf: [0, 1, 3, 4, 6, 7],
        });
    }
...
window.dtFeature = (table, api, op) => {
    downloadPdf({ ...op, btn: 'downloadFeaturePdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadFeatureExcel', dataTable: 'yes' });
};