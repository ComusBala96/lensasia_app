...
$(document).ready(function () {
    ...
    if ($('#dtBreaking').length > 0) {
        ...
        makeAjaxDataTable('dtBreaking', {
            select: false,
            url: 'admin/news/show/breaking/list',
            columns: col_draft,
            pdf: [0, 1, 3, 4, 6, 7],
        });
    }
...
window.dtBreaking = (table, api, op) => {
    downloadPdf({ ...op, btn: 'downloadBreakingPdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadBreakingExcel', dataTable: 'yes' });
};