import { ajaxRequest, csrf_token, domain_url, downloadExcel, downloadPdf, G, makeAjaxDataTable } from '@orian/utils';
    ...
    if ($('#dtAdministrator').length > 0) {
        ...
        let col_draft = [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    if (data.role == 'Lensasia') {
                        return `<a href="${domain_url}admin/users/manage/administrator/edit/${data.uuid}"><span class="p-2 rounded-full shadow-md text-white" style="background-color:#2edcdc;"><i class="fa fa-edit"></i></span></a>`;
                    }
                    return `<a href="${domain_url}admin/users/manage/administrator/edit/${data.uuid}"><span class="p-2 rounded-full shadow-md text-white" style="background-color:#2edcdc;"><i class="fa fa-edit"></i></span></a>
        ...
        makeAjaxDataTable('dtAdministrator', {
            select: true,
            url: 'admin/users/manage/administrator/list',
...
window.dtAdministrator = (table, api, op) => {
    G.deleteAll({
        element: 'deleteAllAdministrator',
        script: 'admin/users/manage/administrator/delete',
        confirm: true,
        api,
    });
    G.updateAll({
        element: 'updateAllAdministrator',
        script: 'admin/users/manage/administrator/updateRow',
        confirm: true,
        dataCols: {
            key: 'ids',
            items: [
    ...
    downloadPdf({ ...op, btn: 'downloadAdministratorPdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadAdministratorExcel', dataTable: 'yes' });
    if ($('.status').length > 0) {
        $('.status').on('click', function () {
            ajaxRequest({
                element: 'status',
                confirm: true,
                script: 'admin/users/manage/administrator/updateStatus',
                type: 'request',
                dataType: 'json',
