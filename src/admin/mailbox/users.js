...
$(document).ready(function () {
    ...
    if ($('#frmSendBulkMailToUsers').length > 0) {
        ...
        ajaxRequest({
            element: 'frmSendBulkMailToUsers',
            validation: true,
            script: 'admin/mailbox/users/bulkMail',
            rules,
            afterSuccess: {
    ...
    if ($('#frmSendMailToUsers').length > 0) {
        $('.save_draft').on('click', function () {
            const draft = $(this).data('draft');
            if ($('#frmSendMailToUsers input[name="draft"]').length === 0) {
                $('<input>').attr({ type: 'hidden', name: 'draft', value: draft }).appendTo('#frmSendMailToUsers');
            } else {
                $('#frmSendMailToUsers input[name="draft"]').val(draft);
            }
            $('.send').click();
        });
        window.addEventListener('beforeunload', function (e) {
            ...
            $('<input>').attr({ type: 'hidden', name: 'draft', value: true }).appendTo('#frmSendMailToUsers');
            if ($('#frmSendMailToUsers input[name="draft"]').length > 0) {
                $('.send').click();
            }
        });
        ...
        ajaxRequest({
            element: 'frmSendMailToUsers',
            validation: true,
            script: 'admin/mailbox/users/create',
            rules,
            afterSuccess: {
                type: 'inflate_reset_response_data',
                afterLoad: () => {
        ...
        let col_draft = [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `<a href="${domain_url}admin/mailbox/users/edit/${data.uuid}"><span class="px-2 py-1 rounded-md shadow-md text-white" style="background-color:#2edcdc;">Compose</span></a>`;
                },
            },
        ];
        makeAjaxDataTable('dtUserMail', {
            select: false,
            url: 'admin/mailbox/users/list',
...
window.dtUserMail = (table, api, op) => {
    downloadPdf({ ...op, btn: 'downloadUserMailPdf', dataTable: 'yes' });
    downloadExcel({ ...op, btn: 'downloadUserMailExcel', dataTable: 'yes' });
};